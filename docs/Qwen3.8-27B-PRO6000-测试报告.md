...
**规律：本机（vLLM 0.21，MTP n=2）MTP 接受率随上下文变长而下降，验证开销却随 KV 变长而上升，
约 30K 之后开 MTP 反而是负优化。** 长上下文生成请用无 MTP 配置。

> 📌 跨设备补充：PRO 5000（vLLM 0.26，MTP n=1，marlin 后端）实测 NVFP4+MTP 在 148K
> 仍有 **+37%**（57.8 t/s，三设备长上下文 decode 最高值），与本机 n=2 配置的崩盘相反。
> 候选变量：n 值（投机 token 数）、后端（marlin vs cutlass）、vLLM 版本。
> 根因分析见 [跨设备横向对比 · MTP 专题](Qwen3.8-27B-跨设备横向对比.md)；本机验证 n=1 是后续 TODO。
