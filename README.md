# Qwen3.8-27B serving benchmark on RTX PRO 6000 Blackwell: vLLM FP8 vs NVFP4

vLLM FP8 与 NVFP4 全面对比 · TTFT / Prefill / Decode / 并发 / 长上下文捞针 / MTP
...
| # | 结论 | 依据 |
|---|---|---|
| 1 | **NVFP4+MTP 是本机最优配置** | decode 132 t/s、8 并发聚合 654 t/s、prefill 全面领先，质量与 FP8 无可见差距 |
| 2 | **MTP 白捡 2.3 倍 decode** | FP8 51→116 t/s，NVFP4 56→132 t/s，接受率 83~100% |
| 3 | **并发扩展极佳** | 8 并发单流仅降 ~10%，聚合吞吐近线性（95→654 t/s） |
| 4 | **100K 长上下文一次答对** | 大海捞针（70% 深度）FP8/NVFP4 均通过 |
| 5 | **NVFP4 需 3 个手工修复** | 详见部署教程 §3；FP8 官方仓 1 个环境变量即可跑通 |
...
| 模型 | [Qwen/Qwen3.8-27B-FP8](https://modelscope.cn/models/Qwen/Qwen3.8-27B-FP8) · [unsloth/Qwen3.8-27B-NVFP4](https://huggingface.co/unsloth/Qwen3.8-27B-NVFP4) |
...
| 并发 | FP8+MTP 单流 / 聚合 | NVFP4+MTP 单流 / 聚合 |
|---|---|---|
| 1 | 79.1 / 78.8 t/s | 95.5 / 95.0 t/s |
| 2 | 70.3 / 137.0 t/s | 77.0 / 151.3 t/s |
| 4 | 77.5 / 299.3 t/s | 91.1 / 346.5 t/s |
| 8 | 73.9 / 556.2 t/s | 86.3 / **654.1 t/s** |

### 🎯 长上下文真实性

大海捞针：96,886 tokens 文本 70% 深度藏入随机密码，FP8 与 NVFP4 均一次答对。
